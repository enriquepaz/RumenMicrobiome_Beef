...
The following calculates alpha diversity metrics and generates alpha rarefaction plots within heifer and steer cohorts.
...
alpha_rare_heifer <- alpha_rare_heifer[1:44,1:6]
colnames(alpha_rare_heifer)[2] <- "seqs.sample"
colnames(alpha_rare_heifer)[3] <- "goods_coverage.ave"
colnames(alpha_rare_heifer)[4] <- "goods_coverage.err"
colnames(alpha_rare_heifer)[5] <- "observed_otus.ave"
colnames(alpha_rare_heifer)[6] <- "observed_otus.err"
#change to numeric variables
cols.num <- c("seqs.sample", "goods_coverage.ave", "goods_coverage.err", "observed_otus.ave", "observed_otus.err")
alpha_rare_heifer[cols.num] <- sapply(alpha_rare_heifer[cols.num],as.numeric)
pd <- position_dodge(width = 275)

heifer_observed_otus <- ggplot(alpha_rare_heifer, aes(x=seqs.sample, y=observed_otus.ave, color=Group, ymin=observed_otus.ave - observed_otus.err, ymax=observed_otus.ave + observed_otus.err)) + geom_line(position=pd, size=1) + geom_pointrange(position=pd) + labs(title="Heifer", x="Sequences per Sample", y = "Observed OTUs") + theme(title=element_text(color="black", size=12, face="bold"), axis.line.x=element_line(color = "black", size = 1), axis.line.y=element_line(color = "black", size = 1), axis.ticks=element_line(color="black"), axis.text=element_text(color="black", size=12, face="bold"), axis.title=element_text(color="black", size=12, face="bold"), legend.text=element_text(face="bold"), legend.title = element_blank()) + scale_colour_manual(values=c("#FF00FF", "#FF0000", "#000080", "#008000")) 
...
alpha_rare_steer <- alpha_rare_steer[1:44,1:6]
colnames(alpha_rare_steer)[2] <- "seqs.sample"
colnames(alpha_rare_steer)[3] <- "goods_coverage.ave"
colnames(alpha_rare_steer)[4] <- "goods_coverage.err"
colnames(alpha_rare_steer)[5] <- "observed_otus.ave"
colnames(alpha_rare_steer)[6] <- "observed_otus.err"
...
steer_observed_otus <- ggplot(alpha_rare_steer, aes(x=seqs.sample, y=observed_otus.ave, color=Group, ymin=observed_otus.ave - observed_otus.err, ymax=observed_otus.ave + observed_otus.err)) + geom_line(position=pd, size=1) + geom_pointrange(position=pd) + labs(title="Steer", x="Sequences per Sample", y = "Observed OTUs") + theme(title=element_text(color="black", size=12, face="bold"), axis.line.x=element_line(color = "black", size = 1), axis.line.y=element_line(color = "black", size = 1), axis.ticks=element_line(color="black"), axis.text=element_text(color="black", size=12, face="bold"), axis.title=element_text(color="black", size=12, face="bold"), legend.text=element_text(face="bold"), legend.title = element_blank()) + scale_colour_manual(values=c("#FF00FF", "#FF0000", "#000080", "#008000")) 

png("figures/FigureS1.png", units = "in", height = 6, width = 12, res = 300)
multiplot(heifer_observed_otus, steer_observed_otus, cols=2)
dev.off()

pdf("figures/FigureS1.pdf", height = 6, width = 12)
multiplot(heifer_observed_otus, steer_observed_otus, cols=2)
dev.off()
```
