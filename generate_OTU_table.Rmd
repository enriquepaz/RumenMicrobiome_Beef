---
title: "Generate OTU Table"
output: html_document
---

The following assembles and quality filters contigs. Quality filtering includes the removal of sequences with ambiguous bases, incorrect length, or improper assemble. Then, the [UPARSE](http://www.drive5.com/uparse/) pipeline is used generate an OTU table with OTUs clustered at 97% identity threshold.

##Assemble Contigs
```{r, engine='bash', results='hide'}
#Create input file for make.contigs 
scripts/make_file.py -d fastq_files/
```

```{r, engine='bash', results='asis'}  
#Default parameters
mothur "#make.contigs(file=fileList.paired.txt, processors=8)"

#Check the summary of the sequences quality
mothur "#summary.seqs(fasta=fileList.paired.trim.contigs.fasta, processors=8)"

#Filter out sequences with ambiguous bases and outside window length 
mothur "#screen.seqs(fasta=fileList.paired.trim.contigs.fasta, maxambig=0, minlength=245, maxlength=275, processors=8)"
```

##Generate OTU table
```{r, engine='bash', results='hide'} 
#Create new fasta files with a header line format compatible with the UPARSE pipeline
python scripts/uparse_format.py -f fileList.paired.trim.contigs.good.fasta -g fileList.paired.contigs.groups
```

```{r, engine='bash', results='asis'}
#Establish permissions for the uparse_python_scripts folder
chmod -R 775 scripts/uparse_python_scripts/

#Find set of unique sequences
$WORK/anaconda2/envs/microbiomeBeef/bin/usearch7.0.1090 -derep_fulllength contigs_for_uparse.fasta -sizeout -output derep.fa

#Sort sequences by decreasing size annotation (cluster size) not including singletons
$WORK/anaconda2/envs/microbiomeBeef/bin/usearch7.0.1090 -sortbysize derep.fa -minsize 2 -output derep.sort.fa

#Generate fasta file containing OTU representative sequences
$WORK/anaconda2/envs/microbiomeBeef/bin/usearch7.0.1090 -cluster_otus derep.sort.fa -otus otus1.fa

#Decompress gold.fasta.gz
gzip -d gold.fasta.gz

#Chimera detection
$WORK/anaconda2/envs/microbiomeBeef/bin/usearch7.0.1090 -uchime_ref otus1.fa -db gold.fasta -strand plus -nonchimeras otus1.nonchimera.fa

#Change header line format
python scripts/uparse_python_scripts/fasta_number.py otus1.nonchimera.fa > otus2.fa

#Map reads to OTUs at 97% identity threshold and generate UCLUST-format tabbed text
$WORK/anaconda2/envs/microbiomeBeef/bin/usearch7.0.1090 -usearch_global contigs_for_uparse.fasta -db otus2.fa -strand plus -id 0.97 -uc otu_map.uc

#Generate OTU table
python scripts/uparse_python_scripts/uc2otutab.py otu_map.uc > otu_table.txt
```