---
Title: "Phylogenetic Tree, PCoA, PERMANOVA, Feed Efficiency Phenotype "
...
##Principal coordinate analysis (heifers vs steers)
...
dev.off()
```
![**Supplementary Figure S1** Principal coordinate analysis (PCoA) using the weighted UniFrac distance matrix displaying significant structuring of bacterial communities between cohorts.](figures/FigureS1.png)
...
#linear model average daily feed intaka (kg/d) for heifer cohort
...
heifer_data_sub <- heifer_data[,c("AnimalID", "Diet", "Selected")]
...
#linear model DMI for steer
lm_ADFI_steer <- lm(ADFI_d_kg ~ perANS +	perHHS + perARS +	perSHS + perBMS + perBRS + perBNS +	perSGS + perBVS +	perCHS + perCAS +	perGVS + perLMS +	perMAS + perSAS + perSMS + perHH + perAN + perSM + perCH + perM2 + perM3 + perRS + perRO, data=steer_data)

summary(lm_ADFI_steer)

#Extract residuals
DMI_res_steer <- resid(lm_DMI_steer) 

#linear model gain for steer
...
#Extract residuals
gain_res_steer <- resid(lm_ADG_steer) 

#Residuals data set for steer
steer_data_sub <- steer_data[,c("AnimalID", "Diet", "Selected")]
...
data_long_heifer <- gather(residuals_data_heifer, measure, residual, DMI_res_heifer:gain_res_heifer)
data_long_steer <- gather(residuals_data_steer, measure, residual, DMI_res_steer:gain_res_steer)

#Generate boxplots
boxplot_heifer <- ggplot(data_long_heifer, aes(x=measure, y=residual)) + geom_boxplot(outlier.color ="#FF0000") + geom_jitter() + labs(title="Outliers Heifer", x="Measure", y="Residuals (kg/d)") + scale_x_discrete(labels=c("Dry Matter Intake", "Gain")) + theme(title=element_text(color="black", size=12, face="bold"), axis.line=element_line(color="black", size=1), axis.ticks=element_line(color="black"), axis.text=element_text(color="black", size=12, face="bold"), axis.title=element_text(color="black", size=12, face="bold"))

boxplot_steer <- ggplot(data_long_steer, aes(x=measure, y=residual)) + geom_boxplot(outlier.color ="#FF0000") + geom_jitter() + labs(title="Outliers Steer", x="Measure", y="Residuals (kg/d)") + scale_x_discrete(labels=c("Dry Matter Intake", "Gain")) + theme(title=element_text(color="black", size=12, face="bold"), axis.line=element_line(color="black", size=1), axis.ticks=element_line(color="black"), axis.text=element_text(color="black", size=12, face="bold"), axis.title=element_text(color="black", size=12, face="bold"))
...
png("figures/FigureS2.png", units="in", height=3, width=12, res=300)
dev.off()
...
quadrant1_heifer <- subset(residuals_data_heifer, DMI_res_heifer > 0 & gain_res_heifer > 0) 
write.table(quadrant1_heifer, file="quadrants/quadrant1_heifer.txt", sep="\t", col.names=T, row.names=F)

quadrant2_heifer <- subset(residuals_data_heifer, DMI_res_heifer < 0 & gain_res_heifer > 0)
write.table(quadrant2_heifer, file="quadrants/quadrant2_heifer.txt", sep="\t", col.names=T, row.names=F)

quadrant3_heifer <- subset(residuals_data_heifer, DMI_res_heifer < 0 & gain_res_heifer < 0)
write.table(quadrant3_heifer, file="quadrants/quadrant3_heifer.txt", sep="\t", col.names=T, row.names=F)

quadrant4_heifer <- subset(residuals_data_heifer, DMI_res_heifer > 0 & gain_res_heifer < 0)
...
quadrant1_steer <- subset(residuals_data_steer, DMI_res_steer > 0 & gain_res_steer > 0) 
write.table(quadrant1_steer, file="quadrants/quadrant1_steer.txt", sep="\t", col.names=T, row.names=F)

quadrant2_steer <- subset(residuals_data_steer, DMI_res_steer < 0 & gain_res_steer > 0)
write.table(quadrant2_steer, file="quadrants/quadrant2_steer.txt", sep="\t", col.names=T, row.names=F)

quadrant3_steer <- subset(residuals_data_steer, DMI_res_steer < 0 & gain_res_steer < 0)
write.table(quadrant3_steer, file="quadrants/quadrant3_steer.txt", sep="\t", col.names=T, row.names=F)

quadrant4_steer <- subset(residuals_data_steer, DMI_res_steer > 0 & gain_res_steer < 0)
...
heifer_plot <- ggplot(residuals_data_heifer, aes(x=ADFI_res_heifer, y=ADG_res_heifer, color=Selected)) + geom_point(size=2.5) + geom_hline(yintercept=0, linetype="dashed", color="black", size=1) + geom_vline(xintercept=0, linetype="dashed", color="black", size=1) + labs(title="Heifer Cohort", x="Average Daily Feed Intake Residuals (kg/d)", y="Average Daily Gain Residuals (kg/d)") + theme(title=element_text(color="black", size=12, face="bold"), axis.line=element_line(color="black", size=1), axis.ticks=element_line(color="black"), axis.text=element_text(color="black", size=12, face="bold"), axis.title=element_text(color="black", size=12, face="bold"), legend.position="none") + scale_color_manual(values=c("#000000", "#FF0000"))

#Steer feed efficiency phenotype plot
steer_plot <- ggplot(residuals_data_steer, aes(x=ADFI_res_steer, y=ADG_res_steer, color=Selected)) + geom_point(size=2.5) + geom_hline(yintercept=0, linetype="dashed", color="black", size=1) + geom_vline(xintercept=0, linetype="dashed", color="black", size=1) + labs(title="Steer Cohort", x="Average Daily Feed Intake Residuals (kg/d)", y="Average Daily Gain Residuals (kg/d)") + theme(title=element_text(color="black", size=12, face="bold"), axis.line=element_line(color="black", size=1), axis.ticks=element_line(color="black"), axis.text=element_text(color="black", size=12, face="bold"), axis.title=element_text(color="black", size=12, face="bold"), legend.position="none") + scale_color_manual(values = c("#000000", "#FF0000"))

png("figures/Figure1.png", units = "in", height = 3, width = 12, res = 300)
