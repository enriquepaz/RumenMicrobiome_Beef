...
```{r, engine='bash'}
sed 's/#SampleID/SampleID/g' mapping_files/mapping_test_set_heifer.txt > r_inputs/mapping_test_set_heifer.txt

sed 's/#SampleID/SampleID/g' mapping_files/mapping_test_set_steer.txt > r_inputs/mapping_test_set_steer.txt
```

```{r}
#Create file for filtering test set for heifer cohort
test_heifer <- read.table("r_inputs/mapping_test_set_heifer.txt", sep="\t", header=T)
test_heifer_sub <- test_heifer[, 1]
write.table(test_heifer_sub, file="filter_files/test_set_heifer_filter.txt", col.names=F, row.names=F, quote=F)

#Create file for filtering test set for steer cohort
cores_steer <- read.table("r_inputs/mapping_test_set_steer.txt", sep="\t", header=T)
cores_steer_sub <- cores_steer[, 1]
write.table(cores_steer_sub, file="filter_files/test_set_steer_filter.txt", col.names=F, row.names=F, quote=F)
```

```{r, engine='bash'}
#Subset discovery test set for heifer cohort
filter_samples_from_otu_table.py -i biom_files/otu_table_final.biom -o biom_files/otu_table_test_heifer.biom --sample_id_fp filter_files/test_set_heifer_filter.txt
biom convert -i biom_files/otu_table_test_heifer.biom -o biom_files/otu_table_test_heifer_json.biom --to-json --table-type="OTU table"

#Subset discovery test set for steer cohort
filter_samples_from_otu_table.py -i biom_files/otu_table_final.biom -o biom_files/otu_table_test_steer.biom --sample_id_fp filter_files/test_set_steer_filter.txt
biom convert -i biom_files/otu_table_test_steer.biom -o biom_files/otu_table_test_steer_json.biom --to-json --table-type="OTU table"
```
...
#Differential OTUs sets for heifer and steer cohorts
differential_otus_heifer <- read.table("differential_otus/heifer_differential_otus.txt", sep="\t", header=F)
differential_otus_steer <- read.table("differential_otus/steer_differential_otus.txt", sep="\t", header=F)

#Arcsine square root function
trans.arcsine <- function(x){asin(sign(x) * sqrt(abs(x)))}

#Arcsine square root transformation for test set heifer cohort
...
#Arcsine square root transformation for test set steer cohort
...
#Create heifer cohort validation set
...
#Create steer cohort validation set
...
#Forward stepwise regression for average daily feed intake (ADFI), average daily gain (ADG), feed efficiency (ADG/ADFI)
...
mapping_heifer <- read.table("r_inputs/mapping_test_set_heifer.txt", sep="\t", header=T)
...
heifer_ADG <- merge(gain_res_heifer,heifer_otus,by="row.names")
...
mapping_steer <- read.table("r_inputs/mapping_test_set_steer.txt", header=T, sep = "\t")
...
gain_res_steer <- as.data.frame(resid(lm_gain_steer))
...
steer_ADG <- merge(gain_res_steer,steer_otus,by="row.names")
...
validation_otus_heifer <- read.table("stepwise_reg/validation_heifer_final.txt", header=T, sep = "\t")
...
validation_heifer_ADG <- merge(gain_res_steer,validation_otus_heifer,by="row.names")
...
validation_otus_steer <- read.table("stepwise_reg/validation_steer_final.txt", header=T, sep = "\t")
...
validation_steer_ADG <- merge(gain_res_heifer,validation_otus_steer,by="row.names")
